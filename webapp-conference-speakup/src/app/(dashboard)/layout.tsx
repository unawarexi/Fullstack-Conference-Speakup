...
function TopBar() {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-border bg-card/80 backdrop-blur-lg px-6">
      ...
      <h1 className="hidden lg:block text-lg font-semibold text-textPrimary">{pageTitle}</h1>

      <div className="flex items-center gap-3">
        {/* Theme toggle */}
        <button
          onClick={() => setTheme(theme === "dark" ? "light" : "dark")}
          className="flex h-9 w-9 items-center justify-center rounded-xl text-textSecondary hover:bg-surfaceLight transition-colors"
        >
        ...
        <Link
          href="/notifications"
          className="relative flex h-9 w-9 items-center justify-center rounded-xl text-textSecondary hover:bg-surfaceLight transition-colors lg:hidden"
        >
...
function MobileNav() {
  const pathname = usePathname();
  const mobileItems = navItems.slice(0, 5); // Home, Meetings, Chat, Recordings, Notifications
